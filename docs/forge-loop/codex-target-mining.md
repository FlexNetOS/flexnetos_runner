...
| `developers.openai.com/codex/permissions` | Permission profiles are beta and must not be mixed with active `sandbox_mode`; use them as least-privilege merge/cleanup contracts with deny rules for secrets, writable git/worktree metadata, Nix-profile runtime proof, stale-shadow cleanup paths, and scoped network allowlists. | `.codex/permissions/forge-loop-workspace.toml` mirrors the active `.codex/config.toml` profile; both keep env/secret/token deny globs while granting `.git`, `/home/flexnetos/meta/.worktrees`, `/home/flexnetos/Downloads`, `/home/flexnetos/.nix-profile`, and stale `.local` cleanup surfaces needed for merge-convergence sessions. |

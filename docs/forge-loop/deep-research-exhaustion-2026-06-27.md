...
- `.github/codex/schemas/forge-loop-output.schema.json` also requires `recommended_self_upgrade` and `tests_required_before_merge`, so the research result is captured as structured evidence.

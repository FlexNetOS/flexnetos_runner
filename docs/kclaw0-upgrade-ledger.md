...
**Net after cycle 14:** quarantine (11) + deadline (12) + secret redaction (13) + windowed rate-limit
(14) applied. The queued backlog is now **pre-dispatch content/injection scan → history-calibrated
risk score** — both unblocked, in-scope runner-plane work. Content/injection scan is the next pick (an
injection-pattern scan of the spec's string fields → `ScanReport{severity, findings}` → fail-closed
reject at/after `lint`). Adoption-ownership remains the only newly-surfaced item gated on another
component (the P3 reuse path).

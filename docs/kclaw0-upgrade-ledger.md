...
**Net after cycle 15:** quarantine (11) + deadline (12) + secret redaction (13) + windowed rate-limit
(14) + content/injection scan (15) applied. The queued backlog is now down to **history-calibrated
pre-route risk score** — the last unblocked, in-scope runner-plane candidate from the cycle-11 sweep
(advice-only: blend a static base rate with the live failure rate from the #3 event ledger to bias
approval/recovery conservative). After it lands, the surfaced backlog is **exhausted** — every
remaining item is blocked on another component (adoption-ownership → P3 reuse path; `verify-commit` /
`staleness` → App/CI seams) or owned elsewhere (weave/atc) — which triggers a fresh deep-research
sweep of the four targets to refill the backlog. Adoption-ownership remains the only newly-surfaced
item gated on another component (the P3 reuse path).

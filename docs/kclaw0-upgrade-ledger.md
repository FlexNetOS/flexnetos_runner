...
**Net after cycle 13:** quarantine (11) + deadline (12) + secret redaction (13) applied. The queued
backlog is now **windowed rate-limit + per-route cooldown → pre-dispatch content/injection scan →
history-calibrated risk score** — all unblocked, in-scope runner-plane work. Rate-limit is the next
pick (a Governor extension; needs clock-injection since the Governor is currently clock-free; distinct
from the per-job retry backoff). Adoption-ownership remains the only newly-surfaced item gated on
another component (the P3 reuse path).

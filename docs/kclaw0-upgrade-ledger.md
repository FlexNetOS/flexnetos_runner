...
| 78 | **Scheduled self-improvement dispatch:** make the green agentic proof keep growing itself | **Agentic System Watch.** A GitHub-hosted scheduled/event-driven workflow now captures run/PR history, runs `agentic-system-audit --strict`, and dispatches `Codex Forge Loop` only when the composite proof is green, `OPENAI_API_KEY` is configured, no PR is open, no PR-local checks are under pressure, and no Codex Forge Loop run is already active. It uploads proof and dispatch-decision artifacts, so the always-research/evaluate/adapt/grow lane is auditable and does not steal local runner lanes or stack self-upgrade PRs before merge. | `.github/workflows/agentic-system-watch.yml`, `docs/forge-loop/agentic-system-proof.md`, `runner-cli::forge_loop` (`agentic-system-audit`, `components-audit`) | GitHub Actions |
...
| 80 | **Codex subscription growth fallback:** remove API-key-only blockage from the growth lane | **Local ChatGPT-authenticated Codex dispatch.** `Codex Forge Loop` now runs on the local self-hosted runner and uses the repo `OPENAI_API_KEY` when present, but falls back to the already-authenticated local Codex CLI/ChatGPT subscription stored in `CODEX_HOME`. `Agentic System Watch` no longer treats a missing API key as a green skip; after a green composite proof and no PR pressure, it dispatches the Codex workflow so self-improvement can actually run from the on-disk subscription auth. | `.github/workflows/codex-forge-loop.yml`, `.github/workflows/agentic-system-watch.yml`, `docs/forge-loop/agentic-system-proof.md`, `runner-cli::forge_loop` (`codex_github_action_workflow_uses_documented_controls`, `agentic_system_watch_dispatches_codex_growth_safely`) | GitHub Actions + local Codex auth |

...
**Cycle 17 applied the top pick (FATAL-first error taxonomy), and cycle 18 applied the next
buildable-now pick (dispatch provenance/authority gate).** The remaining Tier-1 backlog is now
**delegation-target allowlist → max-in-flight concurrency cap (P3-gated)**, plus Tiers 2–3. The next
buildable-now pick is the **delegation-target allowlist** (fail-closed kernel-reachability registry).

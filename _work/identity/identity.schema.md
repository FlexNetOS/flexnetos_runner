...
Secret hygiene rule: identity files must never include tokens, registration secrets, private keys, session material, private auth config, or transient credentials. Do not copy `.runner` `serverUrl` or broker endpoint fields into identity files unless a later review proves they are safe and necessary.
